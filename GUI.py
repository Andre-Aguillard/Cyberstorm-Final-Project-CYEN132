...
class Hangman(Frame):
    ...
    def play(self):
        ...
        response = "Welcome to Galactic Hangman! May I take your\norder?\n\n"\
                 + "Please enter the number of the word you desire\nthe next team to "\
    ...
    def helpMe(self):
        response = "HELP SCREEN ---- type {} to exit. \n\n\n"\
                 + "This is a simple game of Hangman with an outer\nspace twist. \n"\
                 + "You are an Imperial soldier working to protect\nthe Empire's  latest "\
                 + "superweapon, the Death Star! \n\n"\
                 + "Rebel fighters have emerged from space to blow up this "\
                 + "planetary threat, and you must stop them by spelling the "\
                 + "correct word within the time limit.\n\n"\
                 + "Guess letters by typing them into the box below."
        print(response)
        display(response)
