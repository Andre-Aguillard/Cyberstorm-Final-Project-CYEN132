...
############## PLAY FUNCTION WHERE EVERYTHING Begins :) #######################
    ...
    def gameOver(self):
        ...
        Hangman.player_input.delete(0, END)
        # Disable the input so they can't type in letters after they lose.
...
class PopUpWindow(object):
    ...
    def popupWindowWithScrollBar(self):
        ...
        # Creates a listbox that holds all potential words that to be guessed.
...
def newGame(HangmanWord):
    # Set HangmanWord input into the system as THE word and start a new Game
    PopUpWindow.top.destroy()
    global t
    t.destroy()
    t = Hangman(window)
